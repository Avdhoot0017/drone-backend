...
class DashboardService {
  ...
  async getStats(filters?: ObservationFilters): Promise<DashboardStats> {
    ...
    const penaltyRecovered = Number(penaltyStats._sum.penaltyRecovered || 0);
  ...
  async getTrends(
    ...
    >`
      SELECT
        ...
        COALESCE(SUM(p.penalty_recovered), 0) as recovered
      FROM observations o
  ...
  async getRegionStats(filters?: ObservationFilters): Promise<RegionStats[]> {
    ...
    return Array.from(regionMap.values()).map((region) => {
      ...
      const penaltyRecovered = observations.reduce(
        (sum, o) => sum + Number(o.penalty?.penaltyRecovered || 0),
        0
      );
  ...
  async getDistanceDistribution(filters?: ObservationFilters) {
    ...
    >`
      SELECT
        CASE
          WHEN distance_from_coast_km >= 0 AND distance_from_coast_km < 5 THEN '0-5 km'
          WHEN distance_from_coast_km >= 5 AND distance_from_coast_km < 10 THEN '5-10 km'
          WHEN distance_from_coast_km >= 10 AND distance_from_coast_km < 20 THEN '10-20 km'
          WHEN distance_from_coast_km >= 20 AND distance_from_coast_km < 50 THEN '20-50 km'
          WHEN distance_from_coast_km >= 50 AND distance_from_coast_km < 100 THEN '50-100 km'
          ELSE '100+ km'
        END as range_label,
        COUNT(*) as count,
        AVG(distance_from_coast_km) as avg_distance,
        MIN(CASE
          WHEN distance_from_coast_km >= 0 AND distance_from_coast_km < 5 THEN 1
          WHEN distance_from_coast_km >= 5 AND distance_from_coast_km < 10 THEN 2
          WHEN distance_from_coast_km >= 10 AND distance_from_coast_km < 20 THEN 3
          WHEN distance_from_coast_km >= 20 AND distance_from_coast_km < 50 THEN 4
          WHEN distance_from_coast_km >= 50 AND distance_from_coast_km < 100 THEN 5
          ELSE 6
        END) as sort_order
      FROM observations
      WHERE distance_from_coast_km IS NOT NULL
